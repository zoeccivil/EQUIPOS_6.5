#!/usr/bin/env python3
"""
herramientas/subcats_combustible_por_equipo.py
───────────────────────────────────────────────
Script automático que:
  1. Encuentra la categoría COMBUSTIBLE en Firebase.
...
Uso:
    ...
    python herramientas/subcats_combustible_por_equipo.py

    # Aplicar los cambios:
    python herramientas/subcats_combustible_por_equipo.py --apply
...
def main(apply: bool):
    print(SEP)
    print("  SUBCATEGORIAS COMBUSTIBLE => EQUIPOS")
    ...
    # ── 1. Encontrar categoría COMBUSTIBLE ────────────────────────────────────
    categorias = fm.obtener_categorias()
    cat_combustible = next(
        (c for c in categorias if c["nombre"].upper() == "COMBUSTIBLE"), None
    )
    if not cat_combustible:
        print("ERROR: No se encontro la categoria COMBUSTIBLE en Firebase.")
        print("       Crea la categoria primero desde el gestor de la app.")
        sys.exit(1)

    cat_id = cat_combustible["id"]
    print(f"  Categoria COMBUSTIBLE encontrada  (id={cat_id})")
    print()
    ...
    if not apply:
        ...
        print()
        print("  Para aplicar: python herramientas/subcats_combustible_por_equipo.py --apply")
        print(SEP)
        return

    # ── 4. Borrar subcategorías actuales ──────────────────────────────────────
    print(SEP)
    print("  BORRANDO subcategorias existentes...")
    eliminadas = 0
    for s in subcats_actuales:
        try:
    ...
    print("  CREANDO subcategorias por equipo...")
    creadas = 0
    errores = 0
    for e in equipos_sorted:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Reemplaza subcategorias de COMBUSTIBLE con los equipos activos."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
